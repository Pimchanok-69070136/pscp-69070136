...
```text
การคำนวนระยะว่าไปถึงหรือไม่ กระโดดระยะ x แล้วลดลงทีละ 2 แล้วดูว่ามารถ ไปถึงมั๊ย
```
...
```text
กำหนดระยะทางสะสม (total_distance = 0) และนับรอบกระโดด (count = 0) โดยใช้ current_jump เก็บแรงกระโดดเริ่มต้นที่ วนลูปกระโดด: กระโดดไปเรื่อยๆ ตราบใดที่ ระยะรวมยังไม่ถึง และ ยังมีแรงกระโดดเหลืออยู่ (current_jump > 0)ในแต่ละครั้งจะนำระยะกระโดดไปบวกทบเข้า total_distanceลดแรงกระโดดรอบถัดไปลง 2 (current_jump -= 2)บวกเพิ่มจำนวนครั้งที่กระโดดไป 1 (count += 1)สรุปผล: เมื่อออกจากลูป ให้เช็คว่าระยะทางสะสมถึง $y$ หรือไม่ถ้าถึง ให้พิมพ์จำนวนครั้ง countถ้ากระโดดจนหมดแรง (current_jump <= 0) แล้วยังไม่ถึง ให้พิมพ์ -1 (แสดงว่าไม่มีทางกระโดดไปถึงเป้าหมายได้)
```
...
```text
เพื่อเทสว่าถ้าไปไม่ถึงจะได้มั๊ย
```
...
```text
เพื่อเทสว่าสามาถรใช้งานได้มั๊ย
```
